...
4. The moment any inbound message is found → the tag is removed, and the row
   ...
   doesn't change).
5. A cron job (default: every 6 hours) re-checks every `pending` row the same
...
- `GET /run-check-now?secret=<WEBHOOK_SECRET>` — manually trigger a check cycle
  instead of waiting for the cron schedule (works from a browser address bar)
